/* Copia a sitio/ los activos que son fuente de verdad del repo Android:
   - app/src/main/assets/data/*.json        → static/data/
   - app/src/main/res/drawable-nodpi/avatar*.png → static/avatars/{color,bw5,bw1}/
   - app/src/test/resources/fixtures/*.json → src/test/fixtures/

   Uso: npm run assets
   No se versionan las copias de static/ (ver .gitignore); los fixtures sí,
   para que `npm test` funcione sin pasos previos. */
...
const datosDestino = path.join(sitioRoot, 'static', 'data')
const avataresDestino = path.join(sitioRoot, 'static', 'avatars')
const fixturesDestino = path.join(sitioRoot, 'src', 'test', 'fixtures')

const exigir = (ruta) => {
  if (!fs.existsSync(ruta)) {
    console.error(`No existe: ${ruta}`)
    console.error('Corré el script desde sitio/ (npm run assets) con el repo Android completo.')
    process.exit(1)
  }
}
...
console.log(`data: ${nDatos} JSON → static/data/`)
...
console.log(`avatars: color=${conteo.color} bw5=${conteo.bw5} bw1=${conteo.bw1} → static/avatars/`)
