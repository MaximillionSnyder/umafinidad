...
import { AffinityModel } from '../lib/domain/affinity'
import type { Character, Member, Relation } from '../lib/domain/models'
import { parseAptitudes, parseCharacters, parseMembers, parseRelations } from '../lib/data/dtos'

const raizSitio = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '..', '..')

export function leerJson<T>(rutaRelativa: string): T {
  return JSON.parse(readFileSync(path.join(raizSitio, rutaRelativa), 'utf8')) as T
}

export const characters: Character[] = parseCharacters(
  leerJson('static/data/characters.json'),
)
export const relations: Relation[] = parseRelations(
  leerJson('static/data/succession_relation.json'),
)
export const members: Member[] = parseMembers(
  leerJson('static/data/succession_relation_member.json'),
)
export const aptitudes: Map<number, string[]> = parseAptitudes(
  leerJson('static/data/aptitudes.json'),
)
